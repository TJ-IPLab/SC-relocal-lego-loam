...
int failNumber = 0;
const int scale = 10;
const double mismatchThershold = 15.0;
int misflag = 2;
int failflag = 1;
...
cv::Mat SCManager::createSci(Eigen::MatrixXd &scsc)
{
    ...
    {
        const float maxz_sci = 24.2;
        int a = scsc.rows() * scale, b = (scsc.cols()+3) * scale;
        cv::Mat sc = cv::Mat(a, b, CV_8UC1,cv::Scalar(0));
        for (int i = 0; i < scsc.rows(); i++)
            for (int j = 0; j < scsc.cols(); j++)
            {
                if (scsc(i, j) > maxz_sci)
        ...
        Eigen::MatrixXd ringkey (scsc.rows(),1);
        ringkey = makeRingkeyFromScancontext(scsc);
        double maxMean = 0;
        for (int k = 0; k < scsc.rows(); ++k)
        {
            if (ringkey(k,0) > maxMean)
            {
                maxMean = ringkey(k,0);
            }
        }
        for (int i = 0; i != scsc.rows(); ++i)
        {
            uchar pixel = floor(255 * ringkey(i, 0) / maxMean);
            for (int u = i * scale; u < (i + 1) * scale; u++)
                for (int v = 62 * scale; v < (62 + 1) * scale; v++)
                {
                    sc.at<uchar>(u, v) = pixel;
        ...
        cv::Mat sci;
        cv::applyColorMap(sc, sci, cv::COLORMAP_JET);
        for(int u = 0; u != sci.rows; ++u)
            for(int v = 1800; v != 1860; ++v)
            {
                sci.at<uchar>(u, v) = 255;
            }
        // cv::imshow("1",sci);
        // cv::waitKey(0);
        return sci;
